...
ClConnectionStr = 'DRIVER={SQL Server};SERVER=HO-MWFMDB.alfanar.com,1433;DATABASE=Clevest;UID=clevest;PWD=!C13ve$T'

myPM = PlanManage(ClConnectionStr)

conn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER=HO-MWFMDB.alfanar.com,1433;DATABASE=hes;UID=clevest;PWD=!C13ve$T')
...
def ReloadSECData():
    ...
    for filename in SECfiles:
        dfx = pd.read_csv(filename,delimiter=';',header=None, dtype=str,encoding = "utf-8",quoting=csv.QUOTE_NONE, error_bad_lines=False)
        i+=1
        li.append(dfx)
        print ('\r |' + ('#' * i) + ('-' * (len(SECfiles) - i)) + '| File loaded -- > ' + filename , end='')
        # logging.debug('\r |' + ('#' * i) + ('-' * (len(SECfiles) - i)) + '| File loaded -- > ' + filename )
...
def RefreshSM2SMReasons():  
    ...
    conn2 = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER=HO-MWFMDB.alfanar.com,1433;DATABASE=Clevest;UID=clevest;PWD=!C13ve$T')
        ...
        if "data" in SData:
            if SData['data']['Office'] in ActiveSessions[SID]["Areas"]:
                ...
                SiteVisitHTMLStr = '''<form action="../sitevisit/new" method="POST">
                                      ...
                                      <input class="w3-input w3-border" type="text" placeholder="" required name="acc2" hidden id="acc2" style="font-weight: bold; text-align: center;" value="'''+ SData['data']['AccountNumber'] +'''"> </input>
                                    <div style="text-align: center;">
    ...
    while True:
        ...
        while len(OrdersForOpen.keys()) > 0:
            ...
            if len(runningOrders) > 0 :
                ...
                # print("AFTER Clevest Creation IF")
    ...
    if TestAndExtendSession(SID):
        if CheckAppInSession(SID, appTxt):
            if CheckUserAuth(SID, ThisAuth):
                ...
                return render_template("GeneralMessage.html",msgcolor = "lime", MsgTitle = MTitle, MSGBody="Your request has been recieved for premise# ("+ PNum +"), you'll recieve e-mail with the result.", BackTo="/" )
    ...
    if TestAndExtendSession(SID):
        if CheckAppInSession(SID, appTxt):
            if CheckUserAuth(SID, ThisAuth):
                ...
                LEFT(RectificationDate , 12) as 'Rectification Date' ,RectifiedBy,Invest as 'Investigation Comment'
                   ...
                   ,[CloseDate] = '"""+datetime.now().strftime("%Y-%m-%d %H:%M:%S")+"""'"""
                    ...
                    if check != '':
                        ...
                        if  (MainNCRData[0]['Resposability'] == 'SEC') and (os.path.exists("templates/NCRs/" + ncrnum +"/SECApproval"+ncrnum+".pdf") == False):
                            return render_template("GeneralMessage.html",msgcolor = "red", MSGBody="You Must Upload SEC Approval", BackTo="/ncr" )
                    UpdateSubSql = UpdateSql + """ WHERE MainNCRNumber = '"""+ncrnum+"""' and CreatedBy ='"""+uid+"""'"""
...
#def getuserlocation():
#    xconn = pyodbc.connect('DRIVER={SQL Server};SERVER=HO-MWFMDB.alfanar.com,1433;DATABASE=HES;UID=Clevest;PWD=!C13ve$T')
